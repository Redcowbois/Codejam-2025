...
WIFI_StatusTypeDef WIFI_MQTTClientInit(WIFI_HandleTypeDef* hwifi){
	...
	if (hwifi->mqtt.id) {
		// Set user name
		msgLength = sprintf(wifiTxBuffer, "PM=5,%s\r", hwifi->mqtt.id);
		WIFI_SendATCommand(hwifi, wifiTxBuffer, msgLength+1, wifiRxBuffer, WIFI_RX_BUFFER_SIZE);
	}
...
WIFI_StatusTypeDef WIFI_MQTTListen(WIFI_HandleTypeDef* hwifi){
...
WIFI_StatusTypeDef WIFI_PingTarget(WIFI_HandleTypeDef* hwifi){

	int msgLength = 0;
	msgLength = sprintf(wifiTxBuffer, "T0=%s\r", hwifi->remoteIpAddress);
	WIFI_SendATCommand(hwifi, wifiTxBuffer, msgLength+1, wifiRxBuffer, WIFI_RX_BUFFER_SIZE);
