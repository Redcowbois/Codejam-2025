...
SEQUENCE_LENGTH = 35
GEMINI_API_KEY = "AIzaSyDdd2qIRQf8AKl9oitLBL5nPBUMgAScqAM"
...
# Map model index to exercise name
LABEL_MAP = {0: 'IDLE', 1: 'LATERAL RAISE', 2: 'SQUAT'}

# Define configuration for each active exercise
EXERCISE_CONFIG = {
    'LATERAL RAISE': {
        'JOINTS_FOR_CHECK': [
...
# Define the SAVED_JOINTS list (remains static, as it's for model input)
SAVED_JOINTS = [
...
# ... (normalize_landmarks and calculate_angle remain the same) ...
...
def get_llm_feedback(exercise_name, metrics):
    """
    Simulated function for calling the Gemini API to generate detailed,
    conversational feedback based on the accumulated performance metrics.

    In a real application, you would use an HTTP client (like 'requests'
    in Python) to send the prompt to the Gemini API endpoint.
    """
    
    prompt = f"""
    Act as a friendly, expert personal trainer. Analyze the user's last repetition of the '{exercise_name}'.
    Provide concise, actionable feedback (2-3 sentences max) based on the recorded angles.

    Performance Metrics (Lateral Raise Example - Worst/Best observed during the rep):
    - Max Right Shoulder Angle (Target > 60, < 120): {metrics.get('max_r_shoulder', 0.0):.1f} degrees
    - Min Right Elbow Angle (Target > 150): {metrics.get('min_r_elbow', 180.0):.1f} degrees
    - Max Left Shoulder Angle (Target > 60, < 120): {metrics.get('max_l_shoulder', 0.0):.1f} degrees
    - Min Left Elbow Angle (Target > 150): {metrics.get('min_l_elbow', 180.0):.1f} degrees

    If the angles are close, give encouraging advice. If they are poor, suggest a specific correction.
    """
    
    response = requests.post(
  url="https://openrouter.ai/api/v1/chat/completions",
  headers={
    "Authorization": "api-key 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",
...
def run_predictor():
    llm_rep_feedback = "Welcome! Show me a rep when you're ready." #
    ...
    while cap.isOpened():
        ...
        # ------------------------------------------------------------------
        # --- Form Check and Display Logic (Feedback Lockout) ---
        # ------------------------------------------------------------------
        ...
        if display_index != 0 and landmarks_list is not None: 
            ...
            if rep_in_progress:
                # --- REP JUST COMPLETED: Generate LLM Feedback ---
                
                # 1. Gather final metrics for the rep that just finished
                final_metrics = {
                    'max_r_shoulder': current_rep_max_r_shoulder,
                    'min_r_elbow': current_rep_min_r_elbow,
                    'max_l_shoulder': current_rep_max_l_shoulder,
                    'min_l_elbow': current_rep_min_l_elbow,
                }
                
                # 2. Call the LLM (simulated function)
                llm_rep_feedback = get_llm_feedback(
                    LABEL_MAP[LAST_PREDICTED_LABEL], # Use the label of the rep just finished
                    final_metrics
                )
                rep_in_progress = False
        ...
        # Display the LLM Generated Feedback (detailed advice)
        # Handle wrapping/multiline display for the LLM text
        y0, dy = 60, 25
        if (llm_rep_feedback is not None):
            print(llm_rep_feedback)
